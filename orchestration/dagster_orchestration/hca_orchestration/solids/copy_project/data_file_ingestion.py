...
def ingest_data_files(context: AbstractComputeExecutionContext, data_file_paths: set[str]) -> None:
    ...
    control_file_path = _generate_control_file(context, data_file_paths, scratch_config, storage_client)
...
def _generate_control_file(context, data_file_paths, scratch_config: ScratchConfig, storage_client):
    ingest_items = []
    for path in data_file_paths:
        file_bucket_and_prefix = parse_gs_path(path)
        ...
        ingest_items.append(
            f'{{"sourcePath":"{path}", "targetPath":"/{base64.b64decode(blob.crc32c).hex()}/{target_path}"}}')
    control_file_str = "\n".join(ingest_items)
